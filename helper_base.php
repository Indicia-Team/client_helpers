...
class helper_base {
   ...
   *   4xx or 5xx http errors are thrown as a WarehouseRequestException unless
   *   $output_errors is TRUE.
   */
  public static function http_post($url, $postargs = NULL, $output_errors = TRUE) {
    ...
    $httpCode = curl_getinfo($session, CURLINFO_RESPONSE_CODE);
    $curlErrno = curl_errno($session);
    // Check for an error, or check if the http response was not OK.
    if ($curlErrno !== CURLE_OK || $httpCode != 200) {
      ...
      if ($output_errors) {
        ...
        if ($curlErrno) {
          ...
          echo 'Error message: ' . $curlError . '<br/>';
        }
      ...
      // Legacy fallback for other errors - may not be required now
      // WarehouseRequestException is in place.
      $return = [
        'result' => FALSE,
        'output' => $curlErrno ? $curlError : $response,
        'errno' => $curlErrno,
        'status' => $httpCode,
      ];
    }
