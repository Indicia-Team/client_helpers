...
class iform_mnhnl_citizen_science_1 {
    ...
    return array_merge(
      ...
      array(
        ...
        array(
          ...
          'description' => 'List of allowable spatial reference systems, comma separated. Use the spatial ref system ' .
            'code (e.g. OSGB or the EPSG code number such as 4326). Set to "default" to use the settings defined in ' .
  ...
  public static function get_form($args) {
    ...
    if (isset($_GET['taxa_taxon_list_id']) || isset($_GET['taxon_external_key'])) {
      ...
      $species = data_entry_helper::get_population_data(array(
        ...
        'extraParams' => $readAuth + $filter + array(
      ...
      if ($uniqueMeaning) {
        ...
        $speciesCommon = data_entry_helper::get_population_data(array(
            ...
            'extraParams' => $readAuth + array(
              'taxon_meaning_id' => $uniqueMeaning,
        ...
        $nameString = ($species[0]['language_iso'] === 'lat' ? '<em>' : '') . $species[0]['taxon'] .
          ($species[0]['language_iso'] === 'lat' ? '</em>' : '');
  ...
  /**
   * Defines field values to remember after form usage.
   *
   * Hook the indicia_define_remembered_fields method to specify the personal details page as remembered between
   * sessions.
   *
   * @param array $args
   *   Form arguments.
