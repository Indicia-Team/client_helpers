  ...
  var calculateMinAndMax = function () {
    ...
    if (species === '') {
      ...
      for (var k = 0; k < iTM2Opts.triggerEvents.length; k++)
        for (var j = 0; j < 365; j++)
          if (typeof iTM2Data.myData[year][k][j][species] !== 'undefined') {
            if (j < iTM2Data.minDayIndex)
              iTM2Data.minDayIndex = j;
            if (j > iTM2Data.maxDayIndex)
              iTM2Data.maxDayIndex = j;
          }
    } else {
      for (var j = 0; j < 365; j++)
    ...
    if (iTM2Data.minDayIndex > 0)
      iTM2Data.minDayIndex--; // allow for day before data actually starts
