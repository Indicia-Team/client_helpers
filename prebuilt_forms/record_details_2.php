...
[photos]',
  ...
  protected static function get_control_recorddetails($auth, $args, $tabalias, $options) {
    ...
    $availableFields = [
      ...
      'import_records_count' => lang::get('Import record count'),
    ];
   ...
   * details provided by the record details control.
   *
   * Options include:
   ...
   *   data value.
   * * occurrence_attribute_id - required. ID of the attribute to output the
   *   value for.
...
STRUCT;
  ...
  protected static function load_record($auth, $args) {
    if (!isset(self::$record)) {
      ...
      if (!count($records)) {
        $msg = lang::get('The record cannot be found.');
        if (hostsite_get_user_field('training')) {
          $msg .= ' ' . lang::get('If the link you followed was to a non-training record you will need to turn off training mode on your user account profile then try again.');
        }
