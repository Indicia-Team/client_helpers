...
class iform_species_details_2 extends iform_dynamic {
  ...
  private static $default_common_name;
...
[map]',
  ...
  */
  ...
  protected static function get_form_html($args, $auth, $attributes) {
    ...
    hostsite_set_page_title(lang::get('Summary details for {1}', str_replace(['<em>','</em>'], '', self::$preferred)));
    ...
    if (isset(self::$default_common_name)) {
      $taxonNames .= '<input type="hidden" id="species-details-default-common-name" value="' . self::$default_common_name .'"/>';
    }
  ...
  protected static function get_names($auth) {
    ...
    elseif (isset($_GET['external_key'])) {
      ...
      self::$external_key=$_GET['external_key'];
    }
    ...
    foreach ($species_details as $speciesData) {
      if ($speciesData['preferred'] === 't') {
        ...
        if (!isset(self::$external_key)) {
          self::$external_key = $speciesData['external_key'];
        }
      }
      ...
      if (!isset(self::$default_common_name) && !empty($speciesData['default_common_name'])) {
        self::$default_common_name = $speciesData['default_common_name'];
      }
    }
    // Remove default common name from $commonNames array
    if (isset(self::$default_common_name)) {
      if (($key = array_search(self::$default_common_name, self::$commonNames)) !== false) {
  ...
  protected static function get_control_speciesdetails($auth, $args, $tabalias, $options) {
    ...
    // Deal with field name translations. This is a temporary workaround - currently dealt with in JS
    // because to do it in PHP would require much more work in the depths of Indicia.
    ...
    $details_report = self::draw_names($auth['read'], $hidePreferred, $hideCommon, $hideOtherCommon, $hideSynonym, $hideTaxonomy);
    ...
    if ($options['includeAttributes']) {
      ...
      $attrs_report = report_helper::freeform_report(array(
        ...
        'bands' => array(array('content' => str_replace(['{class}'], '', '<dt style="white-space:break-spaces;display:none">{caption}</dt><dd>{value}</dd>'))),
        'extraParams' => array(
   ...
   *   The output html.
   */
  protected static function draw_names($auth, $hidePreferred, $hideCommon, $hideOtherCommon, $hideSynonym, $hideTaxonomy) {
    global $indicia_templates;
    $r = '';
    if (!$hidePreferred) {
      $r .= str_replace(
    ...
    if ($hideCommon == FALSE && !empty(self::$default_common_name)) {
      $r .= str_replace(
        array('{caption}', '{value}', '{class}'),
        array(lang::get('Common name'), self::$default_common_name, ''),
        $indicia_templates['dataValue']);
    }
  ...
  /**
   * A hectad overview and leaflet explore map with a button group
  ...
  protected static function get_control_hectadmap($auth, $args, $tabalias, $options) {
    $options = array_merge([
      ...
      'colour2' => '#7570b3',
      'colour3' => '#d95f02',
      'legendmouse' => 'none', //legendInteractivity
      'highstyle' => 'fill-opacity: 1; fill: black', //highlightStyle
      'lowstyle' => 'fill-opacity: 0.2 ', //lowlightStyle
      'info' => '',
      'download' => 'false'
    ], $options);
  ...
  protected static function get_hectadmap_html($auth, $args, $tabalias, $options) {
    ...
    if (!is_numeric($thresh1)) $thresh1 = $defaultThresh1;
    if (!is_numeric($thresh2)) $thresh2 = $defaultThresh2;
    if ($thresh1 < 0) $thresh1 = date('Y') + $thresh1;
    if ($thresh2 < 0) $thresh2 = date('Y') + $thresh2;
    if ($thresh1 >= $thresh2) $thresh1 = $thresh2-1;
    if ($thresh2 <= $thresh1) $thresh2 = $thresh1+1;
  ...
  protected static function get_best_name() {
    if (isset(self::$default_common_name)) {
      return self::$default_common_name;
...
[map]';
  ...
  /**
   * Used to generate a hidden input that specifies a page-wide filter for any
   * ElasticSearch queries used on this page.
