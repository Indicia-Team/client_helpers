...
class iform_dgfm_attr_value_manager extends iform_dynamic {
  ...
  public static function get_form($args, $nid) {
    ...
    data_entry_helper::$javascript .= "indiciaData.occ_attr_website_ajax_proxy_url = '".iform_ajaxproxy_url(null, 'occurrence_attribute_website')."';\n";
...
TXT;
    ...
    /*if (method_exists($this->model, 'get_system_functions')) {
      ...
      $r .= data_entry_helper::select([
        ...
        'disabled' => $disabled_input === 'YES' ? 'disabled' : '',
        'blankText' => '-none-',
        'lookupValues' => self::get_termlist_terms_data($readAuth, $reportingCategoryTermlistId, 'id'),
      ]);
  ...
  /*protected static function draw_existing_attribute_set_drop_down($website_id, $password) {
    $readAuth = data_entry_helper::get_read_auth($website_id, $password);
    $r='';
    ...
    $r .= data_entry_helper::select([
      'extraParams' => $readAuth,
      'nocache' => true,
      'fieldname' => 'existing_attribute_set',
      'label' => lang::get('Existing groups of characters'),
      'table' => 'attribute_set',
      'captionField' => 'title', 
      'valueField' => 'id',
      'blankText' => '-none-',
  ...
  private static function draw_all_grid_and_button($website_id, $password, $checkboxTemplate, $loadForEditingButtonTemplate, 
    ...
    $r .= '<br><small><em>'.lang::get('Click on the edit icon next to a taxon in the grid above to edit the characters associated with that taxon').'</em></small><br>';
    $r .= '<br>';
    return $r;
  }
