...
class iform_dynamic_sample_occurrence extends iform_dynamic {
  ...
  /**
   * The list of attributes loaded for occurrences. Keep a class level variable,
  ...
  public static function get_dynamic_sample_occurrence_definition() {
    return [
      ...
      'description' => 'A data entry form for records (taxon occurrences). Can
        be used for entry of a single record, ticking species off a checklist or
        entering user selected species into a list. Highly customisable.',
      'supportsGroups' => TRUE,
      'recommended' => TRUE,
    ];
  }
   ...
   *  Sort out {common}.
   *
   * The report paging will not be converted to use LIMIT & OFFSET because we
   * want the full list returned so we can display all the occurrences on the
   * map. When displaying transects, we should display children locations as
   * well as parent.
   */
...
TXT;
    $retVal = array_merge(
      ...
      [
        ...
        [
          ...
          'description' => 'If the survey requests an email address, it is sent
            implicitly for logged in users. Check this box to show it
            explicitly.',
          'type' => 'boolean',
          'default' => FALSE,
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'If the survey requests first name and last name or
            any field which matches a field in the users profile, these are
            hidden. Check this box to show these fields. Always show these fields
            if they are required at the warehouse unless the profile module is
            enabled, <em>copy field values from user profile</em> is selected
            and the fields are required in the profile.',
          'type' => 'boolean',
          'default' => FALSE,
          'required' => FALSE,
          'group' => 'User Interface',
        ...
        [
          ...
          'description' => 'Include an icon to allow taxa to be edited after
            they has been entered into the species grid.',
          'type' => 'checkbox',
          'default' => FALSE,
          'required' => FALSE,
          'group' => 'User Interface',
        ...
        [
          ...
          'description' => 'Include an icon which links to a page from each row
            on the species grid e.g. to display details about a taxon. Use the
            "Species grid page link URL" option to provide a URL.',
          'type' => 'checkbox',
          'default' => FALSE,
          'required' => FALSE,
          'group' => 'User Interface',
        ...
        [
          ...
          'description' => 'URL path of the page being linked to by the "Include
            icon to link to another page from a species grid row" option. The
            URL path should not include server details or a preceeding slash
            e.g. node/216. Note that the current version of the software will
            only supply a taxa_taxon_list_id value to this page as the
            parameter.',
          'type' => 'textfield',
          'default' => FALSE,
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'Parameter name used by the page being linked to by
            the "Species grid page link URL" option. Note in the current version
            of the software the value of the parameter will always be a
            taxa_taxon_list_id.',
          'type' => 'textfield',
          'default' => FALSE,
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'Mouse pointer tooltip for the "Include icon to link
            to another page from a species grid row" option.',
          'type' => 'textfield',
          'default' => FALSE,
          'required' => FALSE,
          'group' => 'User Interface',
        ...
        [
          ...
          'description' => 'Name of the report to use to populate the grid for
            selecting existing data from. The report must return a sample_id
            field or occurrence_id field for linking to the data entry form. As
            a starting point, try
            reports_for_prebuilt_forms/dynamic_sample_occurrence_samples for a
            list of samples.',
          'type' => 'string',
          'group' => 'User Interface',
        ...
        [
          ...
          'description' => 'Allow users to save named sites for recall when they
            add records in future. Users are only able to use their own sites.
            To use this option, make sure you include a [location autocomplete]
        ...
        [
          ...
          'description' => 'Method of data entry, one occurrence at a time, via
            a grid allowing entry of multiple occurrences at the same place and
            date, or allow the user to choose.',
          'type' => 'select',
          'options' => [
        ...
        [
          ...
          'helpText' => 'The species list that species can be selected from.
            ...
            doing single occurrence data entry.',
          'type' => 'select',
          'table' => 'taxon_list',
          'valueField' => 'id',
          'captionField' => 'title',
        ...
        [
          ...
          'helpText' => 'The second species list that species can be selected
            ...
            input mode, if you provide a list here as well as in the Species
            List option above, then both will be available for selection from.
            You might like to use this when you need to augment the species
            available from a main species list with a few additional specialist
            taxa for example.',
          'type' => 'select',
          'table' => 'taxon_list',
          'valueField' => 'id',
        ...
        [
          ...
          'helpText' => 'Use this option to enable newly added rows on the
            species grid to have their default data taken from the previous row
            rather than being initially blank. Use the "Columns to include"
            option to specify which columns you wish to include. The data is
            copied automatically when the new row is created and also when edits
            are made to the previous row.',
          'type' => 'checkbox',
          'default' => FALSE,
          'required' => FALSE,
          'group' => 'Species',
        ...
        [
          ...
          'description' => 'Comma seperated list of columns you wish to include
            when using the "New species grid rows use previous row\'s data"
            option. Non-case or white space sensitive. Any unrecognised columns
            are ignored and the images column cannot be copied.',
          'type' => 'textarea',
          'required' => FALSE,
          'group' => 'Species',
        ],
        [
          ...
          'helpText' => 'Tick this box to enable a filter button in the species
            column title which allows the user to control which species groups
            are available for selection when adding new species to the grid,
            e.g. the user can filter to allow selection from just one species
            group.',
          'type' => 'checkbox',
          'default' => FALSE,
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'The type of control that will be available to select
            a single species.',
          'type' => 'select',
          'options' => [
        ...
        [
          ...
          'description' => 'If checked and doing grid based data entry letting
            the recorder add species they choose to the bottom of the grid,
            sub-species will be displayed in a separate column so the recorder
            picks the species first then the subspecies. The species checklist
            must be configured so that species are parents of the subspecies.
            This setting also forces the Cache Lookups option therefore it
            requires the Cache Builder module to be installed on the Indicia
            warehouse.',
          'type' => 'boolean',
          'default' => FALSE,
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'Should a control be present for sensitivity of each
            record?  This applies when using grid entry mode or when using the
            [species attributes] control to output all the occurrence related
            input controls automatically. The [sensitivity] control outputs a
        ...
        [
          ...
          'description' => 'Should occurrences allow images to be uploaded? This
            applies when using grid entry mode or when using the
            [species attributes] control to output all the occurrence related
            input controls automatically. The [photos] control outputs a photos
            input control independently of this setting.',
          'type' => 'boolean',
          'required' => FALSE,
          'default' => FALSE,
          'group' => 'Species',
        ...
        [
          ...
          'description' => 'Provide percentage column widths for each species
            checklist grid column as a comma separated list. To leave a column
            at its default with, put a blank entry in the list. E.g. "25,,20"
            would set the first column to 25% width and the 3rd column to 20%,
            leaving the other columns as they are.',
          'type' => 'string',
          'group' => 'Species',
          'required' => FALSE,
        ],
        [
          ...
          'description' => 'If you want to allow recording for just part of the
            selected list(s), then select which field you will ' .
        ...
        [
          ...
          'description' => 'Taxa can be filtered by entering values into this
            box. Enter one value per line. E.g. enter a list of taxon group
            titles if you are filtering by taxon group. If you provide a single
            taxon preferred name, taxon meaning ID or external key in this box
            and you set the <strong>Allow a single ad-hoc record or a list of
            records</strong> setting to "Only allow entry of one occurrence at a
            time" then the form is set up for recording just this single
            ...
            Interface section of the form\'s Form Structure to move the
            [species] and [species] controls to a different tab and remove the
            =species= tab, especially if there are no other occurrence
            attributes on the form. The "Use URL taxon parameter" option can be
            used to override the filters specified here.',
          'type' => 'textarea',
          'required' => FALSE,
          'group' => 'Species',
        ],
        [
          ...
          'description' => 'Message which displays the species you are recording
            against in single species mode. When selected, this will
            automatically be displayed where applicable.',
          'type' => 'boolean',
          'required' => FALSE,
          'default' => FALSE,
          'group' => 'Species',
        ...
        [
          ...
          'description' => 'Select the filter to apply to the species names
            which are available to choose from.',
          'type' => 'select',
          'options' => [
        ...
        [
          ...
          'description' => 'You can mark some blocks of the form to only be
            shown as a popup when a certain species is entered into the species
            grid. For each popup block, put the species name on a newline,
            followed by | then the outer block name, followed by | then the
            inner block name if relevant. For example, "Lasius niger|Additional
            info|Colony info" pops up the controls from the block Additional
            Info > Colony info when a species is entered with this name. For the
            species name, specify the preferred name from list.',
          'type' => 'textarea',
          'required' => FALSE,
          'group' => 'Species',
        ],
        [
          ...
          'captionField' => 'term',
          'valueField' => 'id',
          'extraParams' => ['termlist_external_key' => 'indicia:sample_methods'],
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'Supply a list of field names that should be
            remembered in a cookie, saving re-inputting them if they are likely
            to repeat. For greater flexibility use the @lockable=true option on
        ...
        [
          ...
          'description' => 'Set to the name of a permission which is required in
            order to be able to edit other people\'s data.',
          'type' => 'text_input',
          'required' => FALSE,
        ...
        [
          ...
          'description' => 'Set to the name of a permission which is required in
            order to be able to view other people\'s data (not edit).',
          'type' => 'text_input',
          'required' => FALSE,
  ...
  protected static function get_form_html($args, $auth, $attributes) {
    ...
    if (method_exists(self::$called_class, 'build_grid_autocomplete_function')) {
      call_user_func([self::$called_class, 'build_grid_autocomplete_function'], $args);
    }
    else {
      $opts = [
    ...
    if (!empty(data_entry_helper::$entity_to_load['sample:group_id'])) {
      self::$group = data_entry_helper::get_population_data([
        ...
        'extraParams' => $auth['read'] + [
          'view' => 'detail',
          'id' => data_entry_helper::$entity_to_load['sample:group_id'],
        ],
      ]);
      ...
      if (empty($args['location_boundary_id'])) {
        ...
        // limit the map extent. This code grabs the first available value from
        // the list of fields that could hold the value.
  ...
  /**
   * Determine whether to show a grid of existing records or a form for either
   * adding a new record, editing an existing one, or creating a new record from
   * an existing one.
   *
   * @param array $args
   *   Iform parameters.
   ...
   *
   * @return int
   *   The mode [MODE_GRID|MODE_NEW|MODE_EXISTING|MODE_CLONE].
   */
  protected static function getMode(array $args, $nid) {
   ...
   *   HTML for grid.
   */
  protected static function getGrid(array $args, $nid, array $auth) {
    $r = '';
    $attributes = self::getAttributesForEntity('sample', $args, $auth['read'], FALSE);

    $tabs = ['#sampleList' => lang::get('LANG_Main_Samples_Tab')];
    ...
    if (method_exists(self::$called_class, 'getExtraGridModeTabs')) {
      $extraTabs = call_user_func(
          [self::$called_class, 'getExtraGridModeTabs'], FALSE, $auth['read'], $args, $attributes);
    ...
    if (count($tabs) > 1) {
      ...
      $r .= data_entry_helper::tab_header(['tabs' => $tabs]);
    }

    // Here is where we get the table of samples.
    $r .= "<div id=\"sampleList\">" . call_user_func(
        [self::$called_class, 'getSampleListGrid'], $args, $nid, $auth, $attributes) . "</div>";

    // Add content to extra tabs that derived classes may have added.
    if (method_exists(self::$called_class, 'getExtraGridModeTabs')) {
      $r .= call_user_func([self::$called_class, 'getExtraGridModeTabs'], TRUE, $auth['read'], $args, $attributes);
    }
   ...
   *
   * When displaying a grid of occurrences, just load the sample and
   * data_entry_helper::species_checklist will load the occurrences. When
   * displaying just one occurrence we must load the sample and the occurrence.
   */
  protected static function getEntity(&$args, $auth) {
    data_entry_helper::$entity_to_load = [];
    if ((call_user_func([self::$called_class, 'getGridMode'], $args))) {
      ...
      if (self::$loadedOccurrenceId && !self::$loadedSampleId) {
        ...
        if (count($response) !== 0) {
          ...
          if ($args['taxon_filter']) {
            // If a taxon filter is set, check to see if the specified
            // occurrence would pass the filtering. If not, then turn filtering
            // off for this occurrence otherwise it cannot be edited.
            $filtered_taxa = data_entry_helper::get_population_data([
              'table' => 'taxa_search',
              'extraParams' => $auth['read'] + [
      ...
      // Single record entry mode. We want to load the occurrence entity and to
      // know the sample ID.
      if (self::$loadedOccurrenceId) {
        ...
        if (isset($args['multiple_occurrence_mode']) && $args['multiple_occurrence_mode'] === 'either') {
          // Loading a single record into a form that can do single or multi.
          // Switch to multi if the sample contains more than one occurrence.
          $response = data_entry_helper::get_population_data([
            'table' => 'occurrence',
            'extraParams' => $auth['read'] + [
          ...
          if (count($response) > 1) {
            ...
            // Swapping to grid mode for edit, so use species list as the grid's
            // extra species list rather than load the whole lot.
    ...
    if (self::$loadedSampleId) {
      ...
      // If there is a parent sample and we are not force loading the child
      // sample then load it next so the details overwrite the child sample.
    ...
    // Ensure that if we are used to load a different survey's data, then we get
    // the correct survey attributes. We can change args because the caller
    // passes by reference.
   ...
   *
   * @param string $entity
   *   Associated entity name, either sample or occurrence.
   * @param array $args
   *   Form configuration arguments.
   * @param array $readAuth
   *   Authorisation tokens.
   * @param int $id
   *   ID of the sample or occurrence record being reloaded if relevant.
   *
   * @return array
   *   List of attribute definitions
   */
  protected static function getAttributesForEntity($entity, array $args, array $readAuth, $id = NULL) {
    ...
    // Select only the custom attributes that are for this sample method or all
   ...
   * system in to building a form for a new record with default values from the
   * entity_to_load. This feels like it could be easily broken by changes to how
   * the form is built, particularly the species checklist. I would have
   * preferred to modify the completed html but I perceived a problem with
   * multi-value inputs and knowing whether to replace e.g. smpAttr:123:12345
   * with smpAttr:123 or smpAttr:123[]
   *
   * At the time of calling, the entity_to_load contains the sample and the
   * $attributes array contains the sample attributes. No occurrences are
   * loaded. This function calls preload_species_checklist_occurrences which
   * loads the occurrence and occurrence attribute information in to the
   * entity_to_load. Having modified the occurrence information in
   * entity_to_load the species checklist must be called with
   * option['useLoadedExistingRecords'] = true so that the modifications are not
   * overwritten.
   */
  protected static function cloneEntity($args, $auth, &$attributes) {
    ...
    foreach ($attributes as $attributeKey => $attributeValue) {
      if ($attributeValue['multi_value'] == 't') {
        // Set the attribute fieldname to the attribute id plus brackets for
        // multi-value attributes.
        $attributes[$attributeKey]['fieldname'] = $attributeValue['id'] . '[]';
        foreach ($attributeValue['default'] as $defaultKey => $defaultValue) {
          // Fixed a problem with a checkbox_group that the client reported as
          // not saving after cloning. The problem is the value field was also
          // including the fieldname which was preventing save, so I have
          // removed the fieldname from the defaults list here. I don't have
          // time to test all the scenarios for this, so to be safe I have just
          // made it so the fix is only applied to the checkbox_group, if we
          // find there are problems with other types of multi-value control
          // then this check can be removed, but as we only have one reported
          // issue and I can't test all the scenarios I have left in this
          // checkbox_group check to avoid breaking existing code that I can't
          // test.
          ...
          // Set the fieldname in the defaults array to the attribute id plus
          // brackets as well.
          else {
    ...
    // If using a species grid $entity_to_load will now contain elements in the
    // form
    ...
    // Unset the sample and occurrence id from entitiy_to_load as for a new
    // record.
...
HTML;
    ...
    elseif ($args['available_for_groups'] && !isset(data_entry_helper::$entity_to_load['sample:id'])) {
      // Group enabled form being used to add new records, but no group
      // specified in URL path, so give the user a chance to pick from their
      // list of possible groups for this form. Get the list of possible groups
      // they might be posting into using this form. To do this we need the page
      // path without the initial leading /.
      $reload = data_entry_helper::get_reload_link_parts();
      // Slightly messy path handling. Ideally we'd call the same code as
      // group_edit::get_path but we don't know the nid.
  ...
  /**
   * Get the control for map based species input, assumed to be multiple entry:
   * ie a grid. Can be single species though. Uses the normal species grid, so
   * all options that apply to that, apply to this. An option called
   * sampleMethodId can be used to specify the sample method used for
   * subsamples, and therefore the controls to show for subsamples.
   */
   ...
   *
   * @return array
   *   List of items to filter against, e.g. species names or meaning IDs.
   */
  protected static function get_species_filter(array $args) {
  ...
  protected static function get_single_species_data($auth, $args, $filterLines) {
    // The form is configured for filtering by taxon name, meaning id or
    // external key. If there is only one specified, then the form cannot
    // display a species checklist, as there is no point. So, convert our
    // preferred taxon name, meaning ID or external_key to find the preferred
    // taxa_taxon_list_id from the selected checklist.
  ...
  protected static function get_control_species($auth, $args, $tabAlias, $options) {
    ...
    $args['taxon_filter'] = implode("\n", $filterLines);
    // Single species mode only ever applies if we have supplied only one filter
    // species and we aren't in taxon group mode.
    if ($args['taxon_filter_field'] !== 'taxon_group' && count($filterLines) === 1 && ($args['multiple_occurrence_mode'] !== 'multi')) {
      ...
      elseif (count($response) == 1) {
        // Keep the id of the single species in a hidden field for processing if
   ...
   *   Extra parameters array, pre-configured with filters for taxa and name
   *   types.
   * @param array $options
   ...
   */
  protected static function get_control_species_single(array $auth, array $args, array $extraParams, array $options) {
    $r = '';
    ...
    elseif ($args['extra_list_id'] !== '' && $args['list_id'] !== '') {
      $extraParams['taxon_list_id'] = json_encode([
        $args['list_id'],
        $args['extra_list_id'],
      ]);
    }
    ...
    if (isset($options['taxonGroupSelect']) && $options['taxonGroupSelect']) {
      ...
      if (!empty(data_entry_helper::$entity_to_load['occurrence:taxa_taxon_list_id'])) {
        ...
        $species = data_entry_helper::get_population_data([
          ...
          'extraParams' => $auth['read'] + [
            'id' => data_entry_helper::$entity_to_load['occurrence:taxa_taxon_list_id'],
          ],
        ]);
      ...
      $r .= data_entry_helper::select([
        ...
        'valueField' => 'id',
        'captionField' => 'title',
        'extraParams' => $auth['read'] + ['taxon_list_id' => $extraParams['taxon_list_id']],
      ]);
      // Update the select box to link to the species group picker. It must be a
      // select box!
    ...
    if ($ctrl !== 'species_autocomplete') {
      ...
      // For controls which don't know how to do the lookup, we need to tell
      // them.
      $species_ctrl_opts = array_merge([
        'table' => 'taxa_search',
        'captionField' => 'taxon',
    ...
    $r .= call_user_func(['data_entry_helper', $ctrl], $species_ctrl_opts);
    return $r;
  }
  ...
  /**
   * Build a JavaScript function  to format the display of existing taxa added
   * to the species input grid when an existing sample is loaded.
   */
  protected static function build_grid_taxon_label_function($args, $options) {
    ...
    // Set up the indicia templates for taxon labels according to options, as
    // long as the template has been left at it's default state.
  ...
  protected static function get_control_samplephoto($auth, $args, $tabAlias, $options) {
    ...
    // each saving its own type of image. This can keep thephotos from each
  ...
  protected static function get_control_speciesattributes($auth, $args, $tabAlias, $options) {
    if (!(call_user_func([self::$called_class, 'getGridMode'], $args))) {
      ...
      $ctrlOptions = ['extraParams' => $auth['read']];
      $attrSpecificOptions = [];
...
JS;
   ...
   */
  private static function getDynamicAttrs(
    $readAuth,
    $surveyId,
    $ttlId,
    $stageTermlistsTermIds,
    $type,
    $options,
    $occurrenceId = NULL,
    $language = NULL
...
JS;
  ...
  protected static function get_control_date($auth, $args, $tabAlias, $options) {
    if ($args['language'] != 'en') {
      // This will autoload the jquery_ui resource. The date_picker does not
      // have access to the args.
    ...
    return data_entry_helper::date_picker(array_merge([
  ...
  protected static function get_control_locationautocomplete($auth, $args, $tabAlias, $options) {
    ...
    $location_list_args = array_merge($options, ['extraParams' => $extraParams]);
    ...
    if ((isset($args['users_manage_own_sites']) && $args['users_manage_own_sites'])
      ...
      // Pass through the group we are recording in plus its parent, if any, so
      // we can show group sites.
    ...
    if (empty($location_list_args['numValues'])) {
      // Set a relatively high number until we sort out the "more" handling like
  ...
  /**
   * Implements the [location url param] control, for accepting the site to
   * record against using a location_id URL parameter.
   *
   * Outputs hidden inputs into the form to specify the location_id for the
   ...
   * reportProvidesOrderBy option should be set to true if the control is
   * populated by a report that provides its own Order By statement, if the
   * reportProvidesOrderBy option is not set in this situation, then the report
  ...
  protected static function get_control_locationselect($auth, $args, $tabAlias, $options) {
    ...
    $location_list_args = array_merge([
  ...
  protected static function get_control_recordernames(
    ...
    if (empty($_GET['sample_id']) && empty($_GET['occurrence_id']) && !empty($options['defaultToCurrentUser']) && $options['defaultToCurrentUser'] == TRUE) {
      ...
      // Need to escape characters otherwise a name like O'Brian will break the
      // page HTML.
  ...
  protected static function get_control_recordstatus($auth, $args) {
    ...
    $values = ['I', 'C']; // Not initially doing V=Verified.
  ...
  /**
   * A checkbox which when selected will set the record to having a Pending
   * release_status. This is useful if (for instance) an untrained user wishes
   * to have their worked checked.
   * Control always defaults to selected.
   * Note this control has not been tested with and probably won't work with
   * sub-samples mode.
   */
  ...
  /**
   * Force senstivity on added records if the selected species in a given
   * scratchpad.
   *
   * Options are:
...
JS;
  ...
  public static function get_submission($values, $args, $nid) {
    ...
    foreach ($extensions as $extension) {
      if (!empty($extension)) {
        ...
        call_user_func($class_method, $values, [&$submission]);
      }
    }
    return($submission);
  }
   ...
   *
   * @return array
   *   List of css files to include for this form.
   */
  public static function get_css() {
    return ['dynamic_sample_occurrence.css'];
  }

  /**
   * Returns true if this form should be displaying a multiple occurrence entry
   * grid.
   */
  protected static function getGridMode($args) {
    // If loading an existing sample and we are allowed to display a grid or
    // single species selector.
    if (isset($args['multiple_occurrence_mode']) && $args['multiple_occurrence_mode'] === 'either') {
      // Either we are in grid mode because we were instructed to externally, or
      // because the form is reloading after a validation failure with a hidden
      // input indicating grid mode.
      ...
      // A form saved using a previous version might not have the
      // multiple_occurrence_mode setting, so default to grid mode=true,
      // or are we fixed in grid mode?
      return (!isset($args['multiple_occurrence_mode'])) ||
        $args['multiple_occurrence_mode'] == 'multi';
    }
  }

  /**
   * When viewing the list of samples for this user, get the grid to insert into
   * the page.
   */
  protected static function getSampleListGrid($args, $nid, $auth, $attributes) {
    ...
    if (method_exists(self::$called_class, 'getSampleListGridPreamble')) {
      $r = call_user_func([self::$called_class, 'getSampleListGridPreamble']);
    }
    else {
      $r = '';
    }
    ...
    $r .= '<form>';
    if (isset($args['multiple_occurrence_mode']) && $args['multiple_occurrence_mode'] == 'either') {
      $r .= '<input type="button" value="' . lang::get('LANG_Add_Sample_Single') . '" onclick="window.location.href=\'' . hostsite_get_url('node/' . $nid, ['new' => '1']) . '\'">';
      $r .= '<input type="button" value="' . lang::get('LANG_Add_Sample_Grid') . '" onclick="window.location.href=\'' . hostsite_get_url('node/' . $nid, ['new' => '1', 'gridmode' => '1']) . '\'">';
    }
    else {
      $r .= '<input type="button" value="' . lang::get('LANG_Add_Sample') . '" onclick="window.location.href=\'' . hostsite_get_url('node/' . $nid, ['new' => '1']) . '\'">';
    }
    $r .= '</form>';
    return $r;
  }
...
TXT;
  ...
  protected static function load_custom_occattrs($readAuth, $surveyId) {
    if (!isset(self::$occAttrs)) {
      self::$occAttrs = self::getAttributesForEntity('occurrence', ['survey_id' => $surveyId], $readAuth,
   ...
   *   ID of the tab's div if this is being loaded onto a div.
   *
   * @return string
   *   HTML for the control.
   */
  ...
  /**
   * Override the default submit buttons to add a delete button where
   * appropriate.
   */
...
/**
 * A hook function to setup remembered fields whose values are stored in a
 * cookie.
 */
