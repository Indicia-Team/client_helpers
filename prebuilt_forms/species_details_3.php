 ...
 * - An Atlas button that links to the Atlas page for the particular species.
 * - numeric Stats of occurrences with the same meaning as the taxon.
 ...
 * - Gets the contents of a description filelocation defined in the page config
 * - Gets the conservation status from a csv file defined in the page config
 * - Displays a list of habitats where the species has been found
 * - Displays a list of counties where the species has been found
 */
class iform_species_details_3 extends BaseDynamicDetails {
  
  /**
   * Default path for storing cached species pages
   *
   * @var cachePath
   *
   */
   
   private static $cachePath = '/sites/default/files/speciescach';
   ...
   */
  public static function get_species_details_3_definition() {
    return [
      'title' => 'View details of a species (3)',
      'category' => 'Utilities',
      'description' => 'A summary view of a species including records. Pass a parameter in the URL called species_name, ' .
      'containing a species scientific name which defines which species to show.',
      'recommended' => TRUE,
    ];
  }
  
  public static function set_theme_settings() {
    $theme = \Drupal::theme()->getActiveTheme();
    $theme_name = $theme->getName();

    // Load theme settings (immutable config object).
    $settings = \Drupal::config("{$theme_name}.settings");

    // Store whether Bootstrap is enabled.
    static::$isBootstrap = $settings->get('cdn_theme') === 'bootstrap';
    ...
    $retVal = array_merge(
      ...
      [
        ...
        [
          ...
          'caption' => 'File path for description',
          'description' => "The directory to look in for the description file",
          'type' => 'string',
          'default' => '',
          'group' => 'Fields for Species details',
        ],	
        [
          'name' => 'conservationstatuspath',
          'caption' => 'Path to conservation designations file',
          'description' => "The file location for the conservation  file",
          'type' => 'string',
          'default' => '',
          'group' => 'Fields for Species details',
        ],		
        ...
        [
          ...
          "&nbsp;&nbsp;<strong>[numericstats]</strong> - stats associated with the occurrence<br/>" .
          "&nbsp;&nbsp;<strong>[habitat]</strong> - habitats associated with the occurrence<br/>" .
...
[map]',
        ...
        [
          'name' => 'atlas_url',
          'caption' => 'Atlas URL',
          'description' => 'When you click on the Atlas this species\' records button you are taken to this URL. Use {rootfolder} as a replacement ' .
        ...
        [
          'name' => 'atlas_param_name',
          'caption' => 'Atlas Parameters',
          'description' => 'Extra default parameters for the atlas. ' .
          'The default provided is correct if your report uses the standard parameters configuration.',
          'type' => 'string',
          'required' => FALSE,
          'default' => '?ds=1001&tg=%s&vc=0',
          'group' => 'User Interface',
        ],
        [
          'name' => 'species_cache',
          'caption' => 'Species cache',
          'description' => 'Write the species page to a cache so that it does not hit the elastic search end points every time',
          'type' => 'boolean',
          'required' => FALSE,
          'default' => TRUE,
          'group' => 'User Interface',          
  ...
  private static function getTaxaTaxonListIdFromName($args,$speciesName) {
		  ...
		  $paramsExact = [
			...
			'preferred_taxon' => $preferredTaxonName, // exact match on preferred scientific name
			'taxon_rank'      => $expectedRank, // Ranke i.e order, genus, family, species
			'preferred'       => 't',                 // API uses 't'/'f'
			'language'        => 'lat',
			'limit'           => 1,
		  ];
		  ...
		  $rowsExact = self::decodeRows($rawExact);
			...
			if (!is_array($rowsExact) || empty($rowsExact[0])) {
				return null; // or throw, or try a fallback search
			}

			$row0 = $rowsExact[0];

			// Prefer the straight taxa_taxon_list_id; fallback to preferred_taxa_taxon_list_id if needed
			if (!empty($row0['taxa_taxon_list_id'])) {
				return (int) $row0['taxa_taxon_list_id'];
			}
			if (!empty($row0['preferred_taxa_taxon_list_id'])) {
				return (int) $row0['preferred_taxa_taxon_list_id'];
			}

			// As a last resort, you can return null and let the caller handle it
			return null;
 ...
 *
 * This method takes raw JSON data and converts it into a PHP array.
 * If the input is not a valid JSON string or is empty, it returns an empty array.
 *
 * @param mixed $raw The raw JSON data to decode. Expected to be a string.
 *
 * @return array The decoded data as an associative array. Returns an empty array on failure.
 */

  private static function decodeRows($raw): array	{
		...
		// If decode failed, or returned non-array JSON (e.g., null/true/42), return []
		if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
			// Optional: log json_last_error_msg()
			return [];
		}
...
/**
 * Executes a query against the Elasticsearch proxy and returns the response data.
 *
 * This method sends the provided query parameters to the Elasticsearch proxy service,
 * which acts as an intermediary between the application and the Elasticsearch cluster.
 * It retrieves the resulting data and returns it for further processing.
 *
 * @param array $params The query parameters to be sent to the Elasticsearch proxy.
 * @param int $nid Optional node ID for context or filtering (default: 0).
 *
 * @return 
*/ 
    protected static function runEsProxyQuery(array $params, $nid = 0) {
      ...
      curl_setopt_array($ch, [
        ...
        // Prevent very slow responses dragging on
        CURLOPT_LOW_SPEED_LIMIT => 1024, // bytes/sec
        CURLOPT_LOW_SPEED_TIME  => 3,    // seconds
      ...
      // ✅ Handle timeout or other failures cleanly
      if ($response === false) {
        $error = curl_error($ch);
        curl_close($ch);

        // Return a predictable failure shape
        return [
          'error' => true,
          'message' => 'ES proxy request failed',
          'detail' => $error,
        ];
      }
...
/**
 * Sends an HTTP request to a specified endpoint and retrieves the response.
 *
 * This method builds and executes a cURL request to the Indicia REST or Data Services API.
 * It supports both GET and POST methods, automatically appending query parameters for GET
 * requests or encoding the request body as JSON for POST requests.
 *
 * @param string $url The API endpoint path (relative to the base URL).
 * @param array|string $params Optional query parameters or request body data.
 *                              If a string is provided for POST, it is sent as-is.
 * @param string $method The HTTP method to use ('GET' or 'POST'). Defaults to 'GET'.
 *
 * @return string The raw response returned by the API.
 */
  private static function fetchData($url, $params = [], $method = 'GET') {

    $connection = iform_get_connection_details();
    $fullUrl = $connection['base_url'] . $url;
    ...
    curl_setopt_array($session, [
      ...
      // 🔒 Timeouts (crawler‑safe)
      CURLOPT_CONNECTTIMEOUT    => 3,
      CURLOPT_TIMEOUT           => 6,

      // Abort very slow transfers
    ...
    if ($response === false) {
      ...
      return false; // or a structured error if preferred
    }
...
/**
 * Builds the HTTP request headers for API calls.
 *
 * Generates an array of headers required for requests to the Indicia REST/Data Services API.
 * Includes the `Content-Type` header and an `Authorization` header constructed from the
 * website ID, secret, and scope obtained from the connection details.
 *
 * @param string $contentType The MIME type for the request body. Defaults to 'application/json'.
 *

 * @return array An array of HTTP headers to include in the request.
 */
	private static function getHttpRequestHeaders($contentType = 'application/json') {

		// Default: REST/Data Services
		$headers = [
...
/**
 * Generates the HTML for the species summary form.
 *
 * This method builds the form markup for displaying species summary details.
 * It determines whether a taxon is specified via URL parameters or form input,
 * applies theme settings, configures species autocomplete options, and retrieves
 * taxon details when available. If no taxon is specified, it falls back to the
 * parent form rendering.
 *
 * Additionally, it:
 * - Retrieves species names and external keys for filtering.
 * - Adds hidden inputs for preferred and common names.
 * - Constructs Elasticsearch filters for taxon-specific queries.
 * - Applies group integration settings if provided.
 * - Updates the page title based on the selected taxon.
 *
 * @param array $args Form configuration arguments.
 * @param array $auth Authentication tokens for API access.
 * @param array $attributes Additional HTML attributes for the form.
 *
 * @return string The generated HTML for the species summary form.
 */
  protected static function get_form_html($args, $auth, $attributes) {
    ...
    if (self::$notaxon) {
      ...
      self::getNames($auth, self::$taxaTaxonListId);
      ...
      // hidden controls.
      $taxonNames = '<input type="hidden" id="species-details-preferred-name" value="' . strip_tags(self::$preferred,'<em>') . '"/>';
...
/**
 * Retrieves and stores all available names and taxonomy details for a given taxon.
 *
 * This method queries Indicia reports and cache tables to obtain species details,
 * including preferred name, synonyms, common names, and taxonomy hierarchy.
 * It also updates internal properties such as taxon meaning ID, taxa taxon list ID,
 * external key, and default common name.
 *
 * Workflow:
 * - Determines the taxon identifier from parameters or URL query.
 * - Fetches species details using the `taxon_names_2` report.
 * - Populates preferred name, synonyms, common names, and taxonomy fields.
 * - Handles special cases for sites where taxa_taxon_list_id may not work.
 *
 * @param array $auth Authentication tokens for API access.
 * @param int|string|null $taxa_taxon_list_id Optional taxon list ID for species lookup.
 *
 * @return void
 */
  protected static function getNames($auth,$taxa_taxon_list_id) {
    ...
    if (!empty($taxa_taxon_list_id)) {
      $extraParams['taxa_taxon_list_id'] = $taxa_taxon_list_id;
      self::$taxaTaxonListId = $taxa_taxon_list_id;
    }
  ...
  protected static function get_control_speciesdetails($auth, $args, $tabalias, $options) {
    ...
    $bootstrapClass1 = "";
    $bootstrapClass2 = "";
    if (static::$isBootstrap) {
      $bootstrapClass  = "panel panel-success";
      $bootstrapClass1 = "panel-heading";
      $bootstrapClass2 = "panel-body";
    }

    // 1) Build the content that belongs inside the <dl>…</dl> list.
    //    This should be JUST dt/dd pairs — no outer div wrappers.
    $contentBody = '';
    if (isset($details_report)) {
      $contentBody .= $details_report;   // dt/dd from drawNames()
    }
    if (isset($attrs_report)) {
      $contentBody .= $attrs_report;     // dt/dd from freeform_report (no extra wrapper)
    }

    // 2) Wrap the dt/dd content using the dataValueList template.
    //    Now {content} is ONLY the dt/dd block, not the whole $r.
    $listBlock = str_replace(
      ['{id}', '{title}', '{content}'],
      ['detail-panel-speciesdetails', '' , $contentBody],
      $indicia_templates['dataValueList']
    );

    // 3) Build the panel shell and insert the listBlock in the BODY, not the heading.
    $panelHtml  = "<div class=\"{$bootstrapClass}\">";
    $panelHtml .=   "<div class=\"{$bootstrapClass1}\"><h3>" . lang::get('Species Details') . "</h3></div>";
    $panelHtml .=   "<div class=\"{$bootstrapClass2}\">" . $listBlock . "</div>";
    $panelHtml .= "</div>";

    // 4) Add the hidden translator input and return.
    $out  = $panelHtml;
    $out .= '<input type="hidden" id="species-details-fields-translate" value="' . $fieldsTranslated . '"/>';
  ...
  protected static function get_control_recsbyyear($auth, $args, $tabalias, $options) {
	...
	if (static::$isBootstrap) {
		$bootstrapClass  = "panel panel-info";
		$bootstrapClass1 = "panel-heading";
		$bootstrapClass2 = "panel-body";
	}
	
    $r = <<<HTML
		  <div class="detail-panel $bootstrapClass" id="detail-panel-recsbyyear" >
        <div class="$bootstrapClass1" >
          <h3>$title</h3>
        </div>
        <div class="$bootstrapClass2" >
          $customScript
...
HTML;
  ...
  protected static function get_control_recsthroughyear($auth, $args, $tabalias, $options) {
    ...
    $r = <<<HTML
	  <div class="detail-panel $bootstrapClass" id="detail-panel-recsthroughyear">
		  <div class=" $bootstrapClass1">
        <h3>$title</h3>
      </div>
      <div class="$bootstrapClass2" >
...
HTML;
  ...
  protected static function get_control_exploremap($auth, $args, $tabalias, $options) {
	...
	if (static::$isBootstrap) {
		$bootstrapClass  = "panel panel-default";
		$bootstrapClass1 = "panel-body";
	}
	
	$r 	= "<div class='$bootstrapClass' >";
    $r .= "<div class='detail-panel $bootstrapClass1' id='detail-panel-exploremap'><h3>" . lang::get('Explore map') . "</h3>";
...
/**
 * Renders the Explore map (Leaflet) configured with Elasticsearch-backed layers.
 *
 * Builds an Elasticsearch source for grid-square records and returns a Leaflet map
 * configured to display all records matching the current filters as a grid map.
 * The map layer is forced enabled, styled as circles, and can switch from grid
 * squares to full geometries at a specified zoom level.
 *
 * Options handled:
 * - extraParams: Array of ES query parameters to pass through to the source.
 * - nid: Node/context identifier used by ES helpers.
 * - switchToGeomsAt: Zoom at which to switch to full geometries (default 14).
 * - minSqSizeKms: Optional minimum grid square size (km).
 * - maxSqSizeKms: Optional maximum grid square size (km).
 *
 * @param array  $auth     Authentication tokens for API access.
 * @param array  $args     Form configuration arguments (unused here but kept for signature parity).
 * @param string $tabalias Tab alias (unused here but retained for compatibility).
 * @param array  $options  Map configuration options (see list above).
 *
 * @return string The rendered HTML/markup for the Leaflet explore map.
 */
  ...
  protected static function get_control_dualmap($auth, $args, $tabalias, $options) {
	...
	$r  = '<div class="$bootstrapClass" >';
    $r .= '<div class="detail-panel $bootstrapClass1" id="detail-panel-dualmap"><h3>' . lang::get('Ftempora distribution of records') . '</h3>';
  ...
  protected static function get_control_hectadmap($auth, $args, $tabalias, $options) {
    ...
    $r .= "<div class='detail-panel $bootstrapClass' id='detail-panel-hectadmap' >";
    $r .= " <div class='$bootstrapClass1' >";
    $r .= "   <h3>" . lang::get($options['title']) . "</h3>";
    $r .= " </div>";	
    $r .= " <div class='$detailClass2' >";	
...
/**
 * Renders the 10 km (hectad) distribution map with configurable time thresholds.
 *
 * If no taxon is currently selected (`self::$notaxon` is true), returns an empty
 * string. Otherwise, this method:
 * - Creates an Elasticsearch composite-aggregation source keyed by 10 km grid
 *   square centres (`location.grid_square.10km.centre`) to derive min/max year.
 * - Injects a custom client script (`populateHectadMap`) to draw the hectad map.
 * - Establishes sensible defaults for two colour-banding thresholds
 *   (default: 2010 and last completed year), with support for negative offsets
 *   (e.g. `-1` means “last year”).
 * - Outputs hidden inputs for colour settings and style options, plus UI controls
 *   for adjusting thresholds, layer priority (recent vs oldest on top), and an
 *   optional image download button.
 * - Shows a note about inclusion/exclusion of unverified records (tied to the
 *   `xu` GET parameter) and provides a small form to toggle that filter.
 *
 * Expected $options keys include:
 * - extraParams (array): ES query parameters.
 * - nid (int|string): Context identifier for ES helpers.
 * - threshold1 (int): First year threshold (default 2010 or computed).
 * - threshold2 (int): Second year threshold (default last completed year).
 * - colour1, colour2, colour3 (string): Hex colours for bands.
 * - legendmouse, highstyle, lowstyle (string): Rendering/UI options.
 * - download (bool): Whether to display the image download button.
 *
 * @param array       $auth     Authentication tokens for API/report access.
 * @param array       $args     Form configuration arguments (retained for parity).
 * @param string      $tabalias Tab alias (retained for compatibility).
 * @param array       $options  Map configuration and rendering options.
 *
 * @return string HTML markup for the hectad map and its controls, or an empty string if no taxon.
 */
  ...
  $r = "<div class='detail-panel $bootstrapClass' id='detail-panel-photos'>";
	$r.= "<div class='$bootstrapClass1' >";
	$r .= " <h3>";
	$r .= "   ".lang::get('Photos and media');
	$r .= " </h3>";
	$r .= "</div>";
	
  $r .= " <div class=' $bootstrapClass2  ". $options['class'] . " '>";
  $r .= "   <ul>";

    if (count($media) === 0) {
      $r .= "     <li><p>No photos or media files available</p></li>";
    }
    else {
  ...
  protected static function get_control_map($auth, $args, $tabalias, $options) {
    ...
    $r = "<div class='$bootstrapClass detail-panel' id='detail-panel-map'>";
	
	$r .= "<div class='$bootstrapClass1' >";
	$r .= "<h3>" . lang::get('Map') . "</h3>";
	$r .= "</div>";

	$r .= "<div class='$bootstrapClass2' >";
...
/**
 * If enabled, attempt to read a cached species page and return it.
 *
 * @return string|null
 *   Cached HTML if available, otherwise NULL.
 */
protected static function read_species_cache($args) {
  ...
  return file_get_contents($absoluteFile) ?: null;

}


/**
 * If enabled, write rendered HTML output to the species cache.
 *
 * @param string $html
 ...
 */
protected static function write_species_cache($args, $html) {
  ...
  $tmp = $absoluteFile . '.tmp';
  file_put_contents($tmp, $html, LOCK_EX);
  rename($tmp, $absoluteFile);

  return $html;
}
  
protected static function getSpeciesCacheFilename(): ?string {
  $slug = self::canonicalSpeciesSlug();
  $key  = self::buildSpeciesCacheKey();
...
protected static function buildSpeciesCacheKey(): ?string {
  $speciesId =
    self::$taxaTaxonListId
    ?? self::$externalKey
    ?? self::$taxonMeaningId
    ?? ($_GET['species_name'] ?? null);

  if (!$speciesId) {
    return null;
  }

  // Only params that change output
  $vary = [
    'xu' => $_GET['xu'] ?? '0',
  ];
  ...
  return sha1($speciesId . '|' . http_build_query($vary));
}

protected static function canonicalSpeciesSlug(): ?string {
  $name =
    self::$preferredPlain
    ?? self::$preferred
    ?? ($_GET['species_name'] ?? null);
  ...
  /**
   * Draw the Atlas button on the page.
   *
   * @return string
   *   The output HTML string.
   */
  protected static function get_control_atlas($auth, $args) {
    if (self::$notaxon) {
      return '';
    }
    
    // ?taxa_taxon_list_id=242590&ds=1001&tg=spider&vc=0
    
    if (!empty($args['atlas_url']) && !empty($args['atlas_param_name'])) {
      $url = $args['atlas_url'];
...
//        $url = '{rootFolder}' . $url;
      ...
      $url .= $args['atlas_param_name']. self::$taxaTaxonListId;
      global $indicia_templates;
    ...
    if (static::$isBootstrap) {
      ...
      $btnLabel = lang::get('Atlas', self::get_best_name());
      $r = <<<HTML
<div id="atlas-button" >
  <a class="$indicia_templates[buttonHighlightedClass] $bootstrapClass"  href="$url">$btnLabel</a><br/>
</div>
HTML;
    }
    else {
...
/**
 * Displays species-specific notes and images if available.
 *
 * This method retrieves descriptive notes and associated images for the currently
 * selected taxon using the `species_notes_and_images` report. Notes are only shown
 * when:
 * - A taxon is selected (`self::$notaxon` is false).
 * - A valid `taxa_taxon_list_id` is available (to ensure specificity).
 *
 * If conditions are met and notes exist, returns an HTML block containing the
 * species notes. Otherwise, returns an empty string.
 *
 * @param array $auth Authentication tokens for API/report access.
 * @param array $args Form configuration arguments (retained for compatibility).
 *
 * @return string HTML markup for the species notes panel, or an empty string if unavailable.
 */
...
/**
 * Control: [descriptionfile]
 * Fetch and render the species description from a text file.
 * - Uses $args['descriptionpath'] (e.g. sites/default/files/atlas/accounts)
 * - Filename is "Genus species.txt" from the preferred scientific name
 * - Renders HTML content (no escaping), and converts "### " headings to <h3>
 * - Only outputs if content is non-blank
 */
  protected static function get_control_descriptionfile($auth, $args, $tabalias, $options) {
	  if (self::$notaxon || empty($args['descriptionpath'])) {
		return '';
	  }

	  // Preferred scientific name already resolved by this point.
	  $speciesname = self::$preferred;
	  if (empty($speciesname)) {
		return '';
	  }

	  // Clean species name: strip tags/entities and collapse whitespace.
	  $clean = trim(preg_replace('/\s+/', ' ', html_entity_decode(strip_tags($speciesname))));
	  $parts = explode(' ', $clean);
	  if (count($parts) < 2) {
      return '';
	  }
...
//	  $content = @file_get_contents($fileUrl);
    ...
    $content = file_get_contents($filePath);
	  ...
	  // Convert "### Title" into <h3>Title</h3> while leaving existing HTML intact.
	  $lines = preg_split("/\r\n|\n|\r/", $content);
	  ...
	  // Optional: turn double newlines into visual breaks (keeps it lightweight).
	  $html = preg_replace("/(\n\s*\n)+/", "<br/><br/>", $html);
	...
	$r = "<div class='$bootstrapClass' ><div class='$bootstrapClass1'></div><div class='$bootstrapClass2' >".$html."</div></div>";
	  // Return raw HTML (no panel wrapper, no caption row).
	  return $r;
	}

/**
 * Control: [conservationstatus]
 * Fetch and render the conservation status from a text file.
 * - Uses $args['conservationstatuspath'] (e.g. /sites/default/files/atlas/conservation/taxon-designations-20231206.csv)
 * - Filename is a list of species with conservation status information
 * - Renders HTML content (no escaping), and converts "### " headings to <h3>
 * - Only outputs if content is non-blank
 */
protected static function get_control_conservationstatus($auth, $args, $tabalias, $options) {
  if (self::$notaxon || empty($args['conservationstatuspath'])) {
    return '<div><br></div>';
  }
  ...
  // Local helper for safe HTML escaping (UTF-8).
  $esc = static function ($value) {
    return htmlspecialchars((string) $value, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
  };

  // Clean species name and take first two words.
  $clean = trim(preg_replace('/\s+/', ' ', html_entity_decode(strip_tags($speciesname))));
  $parts = explode(' ', $clean);
  if (count($parts) < 2) {
    return '<div><br></div>';
  }
  $targetKey = mb_strtolower($parts[0] . ' ' . $parts[1]);

  // Resolve directory path.
  $dirRel = trim((string) $args['conservationstatuspath']);
  $dirAbs = DRUPAL_ROOT . '/' . ltrim($dirRel, '/');

  if (!is_dir($dirAbs) || !is_readable($dirAbs)) {
    return '<div><br></div>';
  }

  // Find newest taxon-designations-*.csv.
  $pattern = rtrim($dirAbs, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'taxon-designations-*.csv';
  $files = glob($pattern);
  if (!$files) {
    return '<div><br></div>';
  }
  ...
  $nameCol = array_search('Designated name', $header, TRUE);
  if ($nameCol === FALSE) {
    fclose($fh);
    return '<div><br></div>';
  }

  // Columns to display (no "Designated name").
  $wantedCols = [
    'Source',
    'Reporting category',
    'Designation',
  ...
  $colIndex = [];
  foreach ($wantedCols as $colName) {
    $idx = array_search($colName, $header, TRUE);
    if ($idx !== FALSE) {
  ...
  while (($row = fgetcsv($fh)) !== FALSE) {
    $designated = trim((string) ($row[$nameCol] ?? ''));
    if ($designated === '') {
      continue;
    }

    $rowParts = preg_split('/\s+/', $designated);
    if (count($rowParts) < 2) {
      continue;
    }
    $rowKey = mb_strtolower($rowParts[0] . ' ' . $rowParts[1]);
    ...
    foreach ($colIndex as $colName => $idx) {
      $item[$colName] = isset($row[$idx]) ? trim((string) $row[$idx]) : '';
    }
    $matches[] = $item;
  }
  ...
  if (!$matches) {
	...
	if (static::$isBootstrap) {
		$bootstrapClass  = "panel panel-danger";
		$bootstrapClass1 = "panel-body table-responsive";
		$bootstrapClass2 = "table table-striped";
		$bootstrapClass3 = "panel-heading";
	}
	
  // Render plain HTML (no repeated species column).
  $out = [];
  $out[] = "<div class='$bootstrapClass' >";
  $out[] = "<div class='$bootstrapClass3' >";
  $out[] = '<h3>Conservation status</h3>';
  $out[] = '</div>';
  $out[] = "<div class='$bootstrapClass1' >"; 
  $out[] = '<div><strong>' . $esc($parts[0] . ' ' . $parts[1]) . '</strong></div>';

  $out[] = "<table class='$bootstrapClass2' style='border-spacing:0 0.75em; border-collapse:separate;' >";
  $out[] = '<thead><tr>'
    . '<th>Source</th>'
...
$rare ='';
  ...
  foreach ($matches as $m) {
    $out[] = '<tr>'
      . '<td>' . $esc($m['Source'] ?? '') . '</td>'
      . '<td>' . $esc($m['Reporting category'] ?? '') . '</td>'
      . '<td><strong>' . $esc($m['Designation'] ?? '') . '</strong></td>'
      . '<td>' . $esc($m['Date designated'] ?? '') . '</td>'
      . '<td>' . $esc($m['Comments'] ?? '') . '</td>'
      . '</tr>';
	  if ($m['Designation']=='Rare') {
		  $rare='<h4 style="color:red">Rare</h4>';
	  }
  }

  $out[] = '</tbody></table><br/>'.$rare;
  $out[] = "</div></div>";
...
/**
 * Displays numeric statistics for the selected species.
 *
 * This method generates an HTML panel showing key statistics for the current taxon,
 * including:
 * - First recorded year
 * - Last recorded year
 ...
 *
 * The stats are retrieved via Elasticsearch aggregations using
 * `get_stats_from_summary()`. If no taxon is selected or no species context is
 * available, returns an empty string.
 *
 * @param array  $auth     Authentication tokens for API/report access.
 * @param array  $args     Form configuration arguments.
 * @param string $tabalias Tab alias (retained for compatibility).
 * @param array  $options  Additional options for stats retrieval.
 *
 * @return string HTML markup for the numeric stats panel, or an empty string if unavailable.
 */
  protected static function get_control_numericstats($auth,$args,$tabalias,$options) {
	...
	if (static::$isBootstrap) {
		...
		$html .= "<div class='detail-panel $bootstrapClass' id='detail-panel-speciesstats' >";		
		$html .= "  <div class='$bootstrapClass1' >";
		$html .= '    <h3>' . lang::get('Stats') . '</h3>';
		$html .= '  </div>';		
		$html .= "  <div class='$bootstrapClass2' >";		
		$html .= '    <dl class="dl-horizontal">';

		if (!empty($national['first_seen'])) {
		  $html .= '        <dt>' . lang::get('First recorded') . '</dt>';
		  $html .= '        <dd>' . htmlspecialchars($national['first_seen']) . '</dd>';
		}

		if (!empty($national['last_seen'])) {
		...
		$html .= '        <dd>' . number_format((int)$national['event_count']) . '</dd>';
...
/**
 * Get stats for the current species context.
 *
 * Options:
 *  - qualityRecordStatuses: array of verification statuses (default ['V','C','R'])
 *  - hasDateEnd: bool, whether the index has event.date_end (default true)
 *  - esIndex: string, ES index name for proxy (e.g. 'occurrence_brc1_v1')
 *  - speciesFilterField: string override for species filter field:
 *      'taxon.meaning_id' | 'taxon.accepted_name.keyword' | 'taxon.accepted_taxon_id.keyword'
 *  - useTVK: bool, use self::$externalKey with 'taxon.accepted_taxon_id.keyword' (if available)
 *  - debug: bool, include raw request/response in 'debug' key
 */
    ...
    $nid = $args['nid'] ?? self::getNidFromAlias('/species');
		...
		if (empty($ttlId)) {
			return [
				'failed',
				'reason' => 'No taxa_taxon_list_id available'
			];
		}

		// Verification statuses
		$statuses = $options['qualityRecordStatuses'] ?? ['V','C','R'];

		// -----------------------------------------
		// 2. Build simple metric aggregations
		// -----------------------------------------
    ...
    $payload = self::runEsProxyQuery($params, $nid);
		...
		$data = $payload;
			...
			$firstDate = $firstIso ? substr($firstIso, 0, 4) : null;
			$lastDate  = $lastIso  ? substr($lastIso, 0, 4) : null;

			$totalVisits = $data['aggregations']['event_count']['value'] ?? 0;
			$totalRecords = $data['aggregations']['record_count']['value'] ?? 0;

			return [
				'national' => [
					'first_seen' => $firstDate,
					'last_seen'  => $lastDate,
					'event_count'      => (int) $totalVisits,
					'record_count'      => (int) $totalRecords,
				],
			];
	}
 ...
 *
 * Retrieves per–vice county metrics (record count, first and last recorded dates)
 * for the current taxon and outputs a responsive HTML table. If no taxon is
 * selected or no data are available, returns an empty string or a muted message.
 * Supports optional Bootstrap styling when enabled.
 *
 * @param array       $auth     Authentication tokens for API/report access.
 * @param array       $args     Form configuration arguments.
 * @param string      $tabalias Tab alias (retained for compatibility).
 * @param array       $options  Additional options passed to the underlying data fetch.
 *
 * @return string HTML markup for the vice counties panel, or an empty string if unavailable.
 */
  protected static function get_control_counties(array $auth, array $args, $tabalias, array $options) {
  if (self::$notaxon) {
    return '';
  }
  ...
  $stats = self::get_counties($auth, $args, $options);
  $counties = $stats['counties'] ?? [];

  $bootstrapClass = '';
  $bootstrapClassBody = '';
  $bootstrapClassHeading = '';

  if (static::$isBootstrap) {
    $bootstrapClass = 'panel panel-info';
    $bootstrapClassBody = 'panel-body';
    $bootstrapClassHeading = 'panel-heading';
  }

  $html = "<div class=\"detail-panel {$bootstrapClass}\" id=\"detail-panel-counties\">";

  if (!empty($counties)) {
    $html .= "<div class=\"{$bootstrapClassHeading}\">";
    $html .= '<h3>' . lang::get('Vice counties') . '</h3>';
    $html .= '</div>';

    $html .= "<div class=\"{$bootstrapClassBody} table-responsive\">";
    $html .= '<table class="table table-striped table-sm">';
    $html .= '<thead><tr>';
    $html .= '<th>' . lang::get('Vice county') . '</th>';
    $html .= '<th class="text-right">' . lang::get('Records') . '</th>';
    $html .= '<th class="text-right">' . lang::get('First') . '</th>';
    $html .= '<th class="text-right">' . lang::get('Last') . '</th>';
    $html .= '</tr></thead><tbody>';

    foreach ($counties as $row) {
      $countyName = Html::escape($row['vice_county'] ?? 'Unknown');
      $count = number_format((int) ($row['count'] ?? 0));
      $first = Html::escape($row['first_seen'] ?? '');
      $last = Html::escape($row['last_seen'] ?? '');

      $html .= '<tr>';
      $html .= "<td>{$countyName}</td>";
      $html .= "<td class=\"text-right\">{$count}</td>";
      $html .= "<td class=\"text-right\">{$first}</td>";
      $html .= "<td class=\"text-right\">{$last}</td>";
      $html .= '</tr>';
    }
    ...
    $html .= '<p class="text-muted">' . lang::get('No vice county data available.') . '</p>';
  }
...
/**
 * Retrieves per–vice county statistics for the current taxon from Elasticsearch.
 *
 * Resolves the current taxa_taxon_list_id, then queries the ES proxy for counts
 * and date ranges per vice county using a nested aggregation on
 * `location.higher_geography` filtered to type "Vice County", with a
 * reverse-nested scope to compute first/last record dates at the parent level.
 *
 * Returns an associative array with a `counties` key containing rows of:
 * - vice_county (string)
 * - count (int) — total parent records for that vice county
 * - first_seen (string, YYYY) — earliest event.date_start
 * - last_seen  (string, YYYY) — latest event.date_start
 *
 * If no `taxa_taxon_list_id` is available, returns a failure array with a reason.
 *
 * @param array $auth    Authentication tokens for API/proxy access.
 * @param array $args    Context arguments (used to resolve node/NID for proxy).
 * @param array $options Optional settings (e.g., qualityRecordStatuses).
 *
 * @return array {
 *   @type array $counties[] {
 *     @type string $vice_county
 *     @type int    $count
 *     @type string $first_seen
 *     @type string $last_seen
 *   }
 * }
 */
  protected static function get_counties($auth, $args, array $options = [])	{
    ...
    $nid = $args['nid'] ?? self::getNidFromAlias('/species');
		...
		$params = [
		  "textFilters"    => new \stdClass(),
		  "numericFilters" => new \stdClass(),
			...
			"by_vice_county" => [
			  ...
			  "aggs" => [
				"vice_county_only" => [
				  "filter" => [
					"term" => [
					  "location.higher_geography.type.keyword" => "Vice County",
					],
				  ],
				  "aggs" => [
					"vice_counties" => [
		...
		$countyBuckets = $data['aggregations']['by_vice_county']['vice_county_only']['vice_counties']['buckets'] ?? [];

		$counties = [];

		foreach ($countyBuckets as $b) {
		  $countyName = $b['key'] ?? 'Unknown';

		  // These live under reverse_nested -> to_parent
		  $firstIso = $b['to_parent']['first_record']['value_as_string'] ?? null;
		  $lastIso  = $b['to_parent']['last_record']['value_as_string'] ?? null;

		  // Skip counties with no dates (min/max didn't return a value)
		  if (empty($firstIso) || empty($lastIso)) {
			continue;
		  }

		  // year only
		  $firstYear = substr($firstIso, 0, 4);
		  $lastYear  = substr($lastIso, 0, 4);

		  // Count of matching parent records for this VC
		  // Prefer doc_count from reverse_nested. Fallback to the optional "records.value" if you used it.
		  $records = (int)($b['to_parent']['doc_count'] ?? 0);
		  if ($records === 0) {
			$records = (int)($b['to_parent']['records']['value'] ?? 0);
		  }

		  $counties[] = [
			'vice_county' => $countyName,
			'count'       => $records,
			'first_seen'  => $firstYear,
			'last_seen'   => $lastYear,
		  ];
		}

		// Optional: sort by record count desc
		usort($counties, fn($a, $b) => $b['count'] <=> $a['count']);
...
/**
 * Renders a panel showing habitat breakdown for the selected species.
 *
 * Retrieves habitat totals for the current taxon via `get_habitats()` and
 * outputs a responsive HTML table listing each habitat and its record count.
 * If no taxon is selected or no data are available, returns an empty string
 * or a muted message. Supports optional Bootstrap styling when enabled.
 *
 * @param array  $auth     Authentication tokens for API/report access.
 * @param array  $args     Form configuration arguments.
 * @param string $tabalias Tab alias (retained for compatibility).
 * @param array  $options  Additional options passed through to data retrieval.
 *
 * @return string HTML markup for the habitat panel, or an empty string if unavailable.
 */
	protected static function get_control_habitat($auth,$args,$tabalias,$options) {
    if (self::$notaxon) {
      return '';
    }
    ...
    if (empty($taxonMeaningId) && empty($speciesName)) {
	  ...
	  $stats = self::get_habitats($auth, $args,  $options );

	$habitats = $stats['habitats'] ?? [];
	...
	if (static::$isBootstrap) {
		$bootstrapClass  = "panel panel-info";
		$bootstrapClass1 = "panel-body ";
		$bootstrapClass2 = "panel-heading ";
	}
	
	$html  .= "<div class='detail-panel $bootstrapClass' id='detail-panel-habitat '>";
	// Habitat breakdown
	if (!empty($habitats)) {
	  $html .= "<div class='$bootstrapClass2' >";
	  $html .= '<h3>' . lang::get('Habitats') . '</h3>';
	  $html .= "</div>";
	  $html .= "<div class='$bootstrapClass1 table-responsive'>";
	  $html .= '<table class="table table-striped table-sm">';
	  $html .= '<thead><tr>';
	  $html .= '<th>' . lang::get('Habitat') . '</th>';
	  $html .= '<th class="text-right">' . lang::get('Records') . '</th>';
	  $html .= '</tr></thead><tbody>';

	  foreach ($habitats as $row) {
		$habitatName = $row['habitat'] ?? 'Unknown';
		$count       = (int)($row['count'] ?? 0);


		$html .= '<tr>';
		$html .= '<td>' . htmlspecialchars($habitatName) . '</td>';
		$html .= '<td class="text-right">' . number_format($count) . '</td>';
		$html .= '</tr>';
	  }

	  $html .= '</tbody></table></div>';
	} else {
	  $html .= '<p class="text-muted">' . lang::get('No habitat data available.') . '</p>';
	}
...
/**
 * Retrieves per‑habitat record counts for the current taxon from Elasticsearch.
 *
 * Resolves the current `taxa_taxon_list_id`, then queries the ES proxy with a
 * terms aggregation on `event.habitat.keyword` to obtain record totals per
 * habitat (including a “Unknown” bucket for missing values). Results are
 * returned as an array sorted by descending record count.
 *
 * If no `taxa_taxon_list_id` is available, returns a failure array with a reason.
 *
 * @param array $auth    Authentication tokens for API/proxy access.
 ...
 * @return array {
 *   @type array $habitats[] {
 *     @type string $habitat Habitat name (or "Unknown").
 *     @type int    $count   Number of matching records.
 *   }
 * }
 */ 
    ...
    $nid = $args['nid'] ?? self::getNidFromAlias('/species');
		...
		$params = [
		  ...
		  "aggs" => [
			"by_habitat" => [
			  "terms" => [
				"field" => "event.habitat.keyword",
				"size"  => 200,              // adjust if you have many habitats
				"missing" => "Unknown",      // bucket for records with no habitat
			  ]
			],
		  ],
		...
		// -----------------------------------------
			...
			$habitatBuckets = $data['aggregations']['by_habitat']['buckets'] ?? [];

			$habitats = [];
			foreach ($habitatBuckets as $b) {
			  $habitatName = $b['key'] ?? 'Unknown';
			  $records     = (int)($b['doc_count'] ?? 0);
	...
	// Optional: sort by record count desc
	usort($habitats, fn($a, $b) => $b['count'] <=> $a['count']);
