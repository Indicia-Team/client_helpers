...
class import_helper_2 extends helper_base {
  ...
  public static array $blockedFields = [
    ...
    'dna_occurrence:occurrence_id',
    'dna_occurrence:fk_occurrence',
    'dna_occurrence:created_by_id',
    'dna_occurrence:fk_created_by',
  ...
  private static function importToReverseDropDown(array $options) {
    ...
    $lookupData = report_helper::get_report_data([
      ...
      'dataSource' => 'library/imports/reversible_imports_list_extra',
    ...
    foreach ($lookupData as $importRow) {
      ...
      $reversableImports[$importRow['import_guid']] = lang::get('{1}, {2} rows Import ID: {3}, Inserted: {4}, Updated: {5}, Errors: {6}', $importRow['import_date_time'], $affected, $importRow['import_guid'], $importRow['inserted'], $importRow['updated'], $importRow['errors']);
    }
    $r = <<<HTML
      <hr>
      <p>
  ...
  private static function reversalModeWarningOrSkipToResult(array $options) {
    ...
    $imports = report_helper::get_report_data([
      'dataSource' => 'library/imports/occurrence_imports_list_extra',
      'readAuth' => $options['readAuth'],
...
HTML;
  ...
  private static function getExistingImports(array $options) {
    $request = parent::$base_url . "index.php/services/import_2/get_existing_imports";
    $request .= '?' . http_build_query($options['readAuth']);
    $response = self::http_post($request, []);
    $existingImports = json_decode($response['output'], TRUE);
    if (!is_array($existingImports)) {
      throw new Exception('Service call to get_existing_imports failed.');
    }
