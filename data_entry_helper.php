...
JS;
   ...
   *     is bound to.
   *   * **id** - Optional. The id to assign to the HTML control. If not
   *     assigned the fieldname is used.
   ...
   *     control.
   *   * **class** - Optional. CSS class names to add to the control. Defaults
   *     to inline when not sortable.
   *   * **table** - Required. Table name to get data from for the select
   *     options.
   *   * **captionField** - Optional. Field to draw values to show in the
   *     control from. Required unless lookupValues is specified.
   *   * **valueField** - Optional. Field to draw values to return from the
   *     control from. Defaults to the value of captionField.
   *   * **otherItemId** - Optional. The termlists_terms id of the checkbox
   *     group item that will be considered as "Other". When this checkbox is
   *     selected then another textbox is displayed allowing specific details
   *     relating to the Other item to be entered. The otherValueAttrId and
   *     otherTextboxLabel options must be specified to use this feature.
   *   * **otherValueAttrId** - Optional. The attribute id where the "Other"
   *     text will be stored, e.g. smpAttr:10. See otherItemId option
   *     description. This attribute's control does not need to be explicitly
   *     added to the form - it will be autogenerated.
   *   * **otherTextboxLabel** - Optional. The label for the "Other" textbox.
   *     See otherItemId, otherValueAttrId option descriptions.
   *   * **extraParams** - Optional. Associative array of items to pass via the
   *     query string to the service. This should at least contain the read
   *     authorisation array.
   *   * **lookupValues** - If the group is to be populated with a fixed list
   *     of values, rather than via a service call, then the values can be
   *     passed into this parameter as an associated array of key=>caption.
   *   * **cachetimeout** - Optional. Specifies the number of seconds before
   *     the data cache times out - i.e. how long after a request for data to
   *     the Indicia Warehouse before a new request will refetch the data,
   *     rather than use a locally stored (cached) copy of the previous
   *     request. This speeds things up and reduces the loading on the Indicia
   *     Warehouse. Defaults to the global website-wide value; if this is not
   *     specified then 1 hour.
   *   * **template** - Optional. If specified, specifies the name of the
   *     template (in global $indicia_templates) to use for the outer control.
   ...
   *     control.
   *   * **captionTemplate** - Optional and only relevant when loading content
   *     from a data service call. Specifies the template used to build the
   *     caption, with each database field represented as {fieldname}.
   *   * **sortable** - Set to true to allow drag sorting of the list of
   *     checkboxes. If sortable, then the layout will be a vertical column of
   *     checkboxes rather than inline.
   ...
   *   The output of this control can be configured using the following
   *   templates:
   *   * **check_or_radio_group** - Container element for the group of
   *     checkboxes.
...
RIJS;
  ...
  */
  public static function species_checklist(array $options) {
    ...
    $taxalist = self::getSpeciesChecklistTaxaList($options, $taxonRows);
    $shiftedTtlIds = [];
    if ((!empty($_GET['sample_id']) || !empty($_GET['occurrence_id'])) &&
      ...
      $preferredFlagsByTtlId = self::getPreferredFlagsByTtlId($taxalist);
      $shiftTaxaOntoNamesWithSameMeaningResults = self::shiftTaxaOntoNamesWithSameMeaning($options['readAuth'], $taxonRows, $preferredFlagsByTtlId);
...
HTML;
        ...
        $sampleKey = self::getExistingSpeciesRowSampleKey($txIdx, $existingRecordId);
        $row .= self::speciesChecklistDateCell($options, $colIdx, $txIdx, $existingRecordId, $sampleKey);
        $row .= self::speciesChecklistSpatialRefCell($options, $colIdx, $txIdx, $existingRecordId, $sampleKey);
        $row .= self::speciesChecklistSpatialRefPrecisionCell($options, $colIdx, $txIdx, $existingRecordId, $sampleKey);
        $row .= self::speciesChecklistSubSampleAttrCells($options, $colIdx, $txIdx, $existingRecordId, $sampleKey);
  ...
  /**
   * Display occurrence against a name with the same meaning.
   *
   * When taxa are loaded onto data entry page in edit mode, we need to display
   * the occurrence against a different name if we can find one with the same
   * meaning and the original is not available.
   *
   * @param array $readAuth
   *   Read authorisation tokens.
   * @param array $allRequestedTaxa
   *   List of taxa taxon list ids and occurrence information if applicable
   *   for all taxa that are being requested by the grid (including preloaded
   *   plus taxa for existing occurrences).
   * @param array $preferredByTtlId
   *   Preferred flag for all the taxa listed, keyed by taxa_taxon_list_id.
   *
   * @return array
   */
  private static function shiftTaxaOntoNamesWithSameMeaning(array $readAuth, array $allRequestedTaxa, array $preferredFlagsByTtlId) {
    $taxaWhichNeedTaxonMeaningIds = [];
    // Reformat requested taxon details so they are keyed by taxa taxon list ID.
    $allRequestedTaxaByTtlId = [];
    foreach ($allRequestedTaxa as $requestedTaxonDetail) {
      $allRequestedTaxaByTtlId[$requestedTaxonDetail['ttlId']] = $requestedTaxonDetail;
      // If the row needs shifting, it will need a taxon meaning ID for comparison later.
      if (!isset($requestedTaxonDetail['taxon_meaning_id']) && isset($requestedTaxonDetail['occId']) && !array_key_exists($requestedTaxonDetail['ttlId'], $preferredFlagsByTtlId)) {
        $taxaWhichNeedTaxonMeaningIds[] = $requestedTaxonDetail['ttlId'];
      }
    }
    // Taxa added to the list for occurrences which are not in the preloaded
    // list and need shifting won't have a taxon meaning ID.
    if (!empty($taxaWhichNeedTaxonMeaningIds)) {
      $fetchTaxonMeaningIds = data_entry_helper::get_population_data([
        'table' => 'taxa_taxon_list',
        'extraParams' => $readAuth + [
          'query' => json_encode(['in' => ['id' => $taxaWhichNeedTaxonMeaningIds]]),
          'view' => 'detail',
        ],
      ]);
      foreach ($fetchTaxonMeaningIds as $taxonMeaningInfo) {
        $allRequestedTaxaByTtlId[$taxonMeaningInfo['id']]['taxon_meaning_id'] = $taxonMeaningInfo['taxon_meaning_id'];
      }
    }
    // Keep track of processed rows.
    $shiftedTtlIds = [];
    // Cycle through all taxa relating to occurences so we can find ones
    // without a taxa_taxon_list on the page then find a taxa_taxon_list_id we
    // can actually use.
    foreach ($allRequestedTaxa as &$possibleRowToShiftFrom) {
      // Will need to shift if an existing occurrence and not in the list of
      // taxa taxon lists available in the grid.
      if (isset($possibleRowToShiftFrom['occId']) && !array_key_exists($possibleRowToShiftFrom['ttlId'], $preferredFlagsByTtlId)) {
        ...
        foreach ($allRequestedTaxa as &$possibleRowToShiftTo) {
          // If row to shift has already been processed then don't do anymore
          // work. This can break the loop when a hit found.
          if (!in_array($possibleRowToShiftFrom['ttlId'], $shiftedTtlIds)) {
            ...
            if ($allRequestedTaxaByTtlId[$possibleRowToShiftFrom['ttlId']]['taxon_meaning_id'] == $allRequestedTaxaByTtlId[$possibleRowToShiftTo['ttlId']]['taxon_meaning_id'] &&
                $possibleRowToShiftFrom['ttlId'] != $possibleRowToShiftTo['ttlId'] &&
                array_key_exists($possibleRowToShiftTo['ttlId'], $preferredFlagsByTtlId) &&
                // Note cannot use TRUE, as boolean is supplied as string
                $preferredFlagsByTtlId[$possibleRowToShiftTo['ttlId']] == 't') {
  ...
  /**
   * Get's the preferred flag for each taxon, keyed by taxa taxon list ID.
   *
   * @param array $taxonListItems
   *   The details of the taxa available to choose from.
   *
   * @return array
   ...
   * @return array
   */
  private static function applyChangesToSwitchedRows(&$rowToShiftFrom, &$rowToShiftTo, &$shiftedTtlIds) {
...
JS;
   ...
   */
  public static function getSpeciesChecklistTaxaList($options, &$taxonRows) {
...
if (errors$uniq.length>0) {
  ...
  /**
   * Find the subsample key for an existing species_checklist occurrence row.
   *
   * The key is the index of the subsample in the entity_to_load concatenated
   * with the sample ID.
   *
   * @param int $txIdx
   *   Taxon row index.
   ...
   *
   * @return string
   *   The subsample key or NULL.
   */
  private static function getExistingSpeciesRowSampleKey($txIdx, $existingRecordId) {
    // When occurrences are loaded, the related sampleIDX is stored.
    $entityIdxKey = "sc:$txIdx:$existingRecordId:occurrence:sampleIDX";
    if (isset(self::$entity_to_load[$entityIdxKey])) {
      // Extract the index of the subsample in the _entity_to_load.
      $entityIdx = self::$entity_to_load[$entityIdxKey];

      //Extract the sub sample ID from the entity_to_load.
      $keys = preg_grep("/^sc:$entityIdx:\d+:sample:id$/", array_keys(self::$entity_to_load));
      if (count($keys)) {
        $key = array_pop($keys);
        $sampleId = self::$entity_to_load[$key];

        return "$entityIdx:$sampleId";
      }
    }
    return NULL;
  }
   ...
   * @param $existingSampleKey string
   *   If an existing sample record, pass the key generated by
   *   getExistingSpeciesRowSampleKey().
   *
   * @return string
   *   HTML to insert into the grid
   */
  private static function speciesChecklistDateCell($options, $colIdx, $rowIdx, $existingRecordId, $existingSampleKey = NULL) {
    $r = '';
    if ($options['datePerRow']) {
      $value = NULL;
      $fieldname = "sc:$options[id]-$rowIdx:$existingRecordId:occurrence:date";
      if ($existingSampleKey) {
        $key = "sc:$existingSampleKey:sample:date_start";
...
HTML;
   ...
   *   HTML to insert into the grid
   */
  private static function speciesChecklistSpatialRefCell($options, $colIdx, $rowIdx, $existingRecordId, $existingSampleKey = NULL) {
    $r = '';
    if ($options['spatialRefPerRow']) {
      $value = '';
      $fieldname = "sc:$options[id]-$rowIdx:$existingRecordId:occurrence:spatialref";
      if ($existingSampleKey) {
        $key = "sc:$existingSampleKey:sample:entered_sref";
...
HTML;
   ...
   *   HTML to insert into the grid
   */
  private static function speciesChecklistSpatialRefPrecisionCell($options, $colIdx, $rowIdx, $existingRecordId, $existingSampleKey = NULL) {
    $r = '';
    if ($options['spatialRefPrecisionAttrId']) {
      $value = '';
      $fieldname = "sc:$options[id]-$rowIdx:$existingRecordId:occurrence:spatialrefprecision";
      if ($existingSampleKey) {
        $key = "sc:$existingSampleKey:sample:sref_precision";
        $value = isset(self::$entity_to_load[$key]) ? self::$entity_to_load[$key] : '';
      }
...
HTML;
   ...
   *   HTML to insert into the grid
   */
  private static function speciesChecklistSubSampleAttrCells($options, $colIdx, $rowIdx, $existingRecordId, $existingSampleKey = NULL) {
    $r = '';
    foreach ($options['subSampleAttrs'] as $subSampleAttrId) {
      if (isset($options['subSampleAttrInfo'][$subSampleAttrId])) {
        $value = NULL;
        if ($existingSampleKey) {
          $key = "sc:$existingSampleKey:sample:smpAttr:$subSampleAttrId";
          $value = isset(self::$entity_to_load[$key]) ? self::$entity_to_load[$key] : NULL;
        }
